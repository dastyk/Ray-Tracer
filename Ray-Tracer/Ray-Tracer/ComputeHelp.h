...
public:
	...
	T* Map()
	{
		D3D11_MAPPED_SUBRESOURCE MappedResource;
		T* p = nullptr;
		if (SUCCEEDED(_D3DContext->Map(_Resource, 0, D3D11_MAP_WRITE_DISCARD, 0, &MappedResource)))
			p = (T*)MappedResource.pData;
...
private:
	...
	bool Init(TCHAR* shaderFile, TCHAR* blobFileAppendix, const char* pFunctionName, D3D10_SHADER_MACRO* pDefines,
...
public:
	...
	~ComputeWrap();
	ComputeShader* CreateComputeShader(TCHAR* shaderFile, TCHAR* blobFileAppendix, const char* pFunctionName, D3D10_SHADER_MACRO* pDefines);

	ID3D11Buffer* CreateConstantBuffer(UINT uSize, VOID* pInitData, bool write = false, const char* debugName = nullptr);

	ComputeBuffer* CreateBuffer(COMPUTE_BUFFER_TYPE uType, UINT uElementSize,
		UINT uCount, bool bSRV, bool bUAV, VOID* pInitData, bool cpuAccess = false, bool bCreateStaging = false, const char* debugName = nullptr);
